<div align="center">
  
# 🚀 Tatil & Seyahat Sitesi

### Murat Yücedağ
Bu proje, **Murat Yücedağ**’ın YouTube kanalında paylaştığı **“Asp.Net Mvc5 ile Tatil & Seyahat Sitesi”** eğitim serisi referans alınarak geliştirilmiştir.

[![ASP.NET Core](https://img.shields.io/badge/ASP.NET%20Core-MVC-blue)](https://dotnet.microsoft.com/)
[![Entity Framework](https://img.shields.io/badge/Entity%20Framework-Code%20First-green)](https://docs.microsoft.com/ef/)
[![ASP.NET Core Identity](https://img.shields.io/badge/ASP.NET%20Core-Identity-512BD4)](https://learn.microsoft.com/aspnet/core/security/authentication/identity)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind%20CSS-Utility--First-38BDF8?logo=tailwindcss&logoColor=white)](https://tailwindcss.com/)

</div>

---

## 🎯 Projenin Amacı
- ASP.NET Core MVC mimarisini daha iyi kavramak  
- Entity Framework Core ile Code First yaklaşımını pekiştirmek  
- CRUD işlemlerini gerçek senaryolar üzerinden uygulamak  
- Validation, paging ve tablo yönetimi gibi sık kullanılan yapıları öğrenmek  
- Proje geliştirme sürecinde temiz ve sürdürülebilir kod yazma alışkanlığı kazanmak  

---

## 🛠 Kullanılan Teknolojiler

### 🔧 Backend
- **Framework:** ASP.NET Core 8.0 MVC  
- **ORM:** Entity Framework Core 8.0 (Code First)  
- **Mimari:** MVC Pattern  
- **Authentication:** ASP.NET Core Identity  
- **Validation:** FluentValidation  
- **Asenkron Programlama:** async / await  
- **Bağımlılık Yönetimi:** Dependency Injection  

### 🎨 Frontend
- **CSS Framework:** Tailwind CSS  
- **JavaScript:** jQuery  
- **Tablo Yönetimi:** DataTables  
- **View Engine:** Razor  

### 🗄️ Veritabanı
- **Veritabanı:** Microsoft SQL Server  
- **Migration Yönetimi:** EF Core Migrations  

### 🧰 Geliştirme Araçları
- **IDE:** JetBrains Rider  
- **Versiyon Kontrol:** Git & GitHub

--- 

# **Görseller**

# 🛠 **Admin Panel**

<img width="1469" height="831" alt="image" src="https://github.com/user-attachments/assets/4352a396-a0a7-4b3e-aa11-58a70a2af954" />
<img width="1466" height="830" alt="image" src="https://github.com/user-attachments/assets/6628aff3-d70a-44e1-8b8d-5ddc958523dc" />
<img width="1469" height="834" alt="image" src="https://github.com/user-attachments/assets/8969e936-825f-4bbe-8ddd-5888d8d405c4" />
<img width="1469" height="828" alt="image" src="https://github.com/user-attachments/assets/45aa7a43-01d2-469e-a241-ac3cb5288930" />
<img width="1469" height="830" alt="image" src="https://github.com/user-attachments/assets/c76215de-2a1e-41ca-aa61-ac63cf11b52b" />

--- 

<img width="1469" height="830" alt="image" src="https://github.com/user-attachments/assets/e9a3f815-3bb7-42f2-a85b-f98b5b9e3167" />
<img width="1466" height="828" alt="image" src="https://github.com/user-attachments/assets/7f2c3807-ee1b-4980-9c7d-44a16b97c396" />

---

<img width="2940" height="3284" alt="image" src="https://github.com/user-attachments/assets/ef5f4473-6099-4c94-ad1d-3575a0ec94ef" />
<img width="2940" height="5786" alt="image" src="https://github.com/user-attachments/assets/a1764af9-25da-4bc3-bb25-31a100389167" />
<img width="2940" height="3949" alt="image" src="https://github.com/user-attachments/assets/9ceea635-fc26-4781-9869-5c86ddfa391e" />
<img width="2940" height="3152" alt="image" src="https://github.com/user-attachments/assets/d3c69aac-f38f-4f6c-826d-805b22500311" />













